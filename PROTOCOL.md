...
| DATA_FRAGMENT | 0x04  | Fragmented data chunk (large messages)   |

### Fragment Payload Format

Used when a single message exceeds 64 KiB. The payload of a `DATA_FRAGMENT` frame is:

```
[4B total_size (LE)][4B offset (LE)][chunk]
```

The receiver reassembles chunks by `offset` until `received == total_size`, then delivers the complete message.
...
| v1.1    | 2026-06-29 | Removed encryption layer (ECDH/AES-GCM); data now sent in plaintext. Removed handshake frames. Renumbered sections. |
