...
public:
    ...
    BoolResult Connect(std::string_view server_name, const uint32_t timeout_ms, uint32_t) override
    {
        ...
        pipe_ = CreateFileA(name.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_FLAG_OVERLAPPED,
                            nullptr);
...
private:
    ...
    {
        DWORD bytes = 0;
        if (const BOOL ok = ReadFile(ctx->pipe, ctx->raw_read_buf, sizeof(ctx->raw_read_buf), &bytes, &ctx->read_ov);
