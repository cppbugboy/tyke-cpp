...
namespace tyke
{
    ...
    void CancelContext::Cancel(const ContextError err) const
    {
        ...
        {
            std::lock_guard<std::mutex> lock(state_->mu);
            if (state_->err != ContextError::kNone) return; // 避免重复取消
            state_->err = err;
    ...
    void TimerContext::Init(ContextPtr parent,
    ...
    void TimerContext::ActivateTimer() {
        ...
        timer_id_ = GetGlobalTimingWheel().AddTaskAt(
            steady_deadline,
            [weak = std::weak_ptr<TimerContext>(
                 std::static_pointer_cast<TimerContext>(shared_from_this()))]() {
    ...
    // ============================================================================
    std::shared_ptr<EmptyContext> ContextPool::EmptyContext()
    {
    ...
    ContextPtr WithCancel(ContextPtr parent)
    {
        ...
        return std::shared_ptr<CancelContext>(
            raw,
    ...
    ContextPtr WithDeadline(ContextPtr parent,
                            const std::chrono::system_clock::time_point deadline)
    {
        auto pooled = ContextPool::AcquireTimer();
        auto* raw = pooled.Get();
        raw->Init(std::move(parent), deadline);
        return std::shared_ptr<TimerContext>(
            raw,
    ...
    ContextPtr WithValue(ContextPtr parent, const void* key, std::any value)
    {
        ...
        return std::shared_ptr<ValueContext>(
            raw,
