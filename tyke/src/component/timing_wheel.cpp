...
namespace tyke
{
    bool TimingWheel::Init(uint32_t base_tick_ms,
                           const std::vector<uint32_t>& slots_per_level)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if (initialized_) return false;
        if (base_tick_ms == 0 || slots_per_level.empty()) return false;
    ...
    {
        if (!cb) return kInvalidTimerId;
        std::lock_guard<std::mutex> lock(mutex_);
        if (!initialized_ || stop_.load(std::memory_order_acquire)) return kInvalidTimerId;
    ...
    TimerId TimingWheel::AddRepeatedTask(uint32_t initial_delay_ms,
                                         uint32_t interval_ms,
                                         std::function<void()> cb)
    {
        if (!cb || interval_ms == 0) return kInvalidTimerId;
        std::lock_guard<std::mutex> lock(mutex_);
        if (!initialized_ || stop_.load(std::memory_order_acquire)) return kInvalidTimerId;
    ...
    std::optional<std::chrono::milliseconds> TimingWheel::GetRemainingTime(TimerId id) const
    {
        std::lock_guard<std::mutex> lock(mutex_);
        auto it = task_map_.find(id);
        if (it == task_map_.end()) return std::nullopt;
    ...
    uint64_t TimingWheel::GetMaxCapacityMs() const
    {
        ...
        for (auto& l : levels_) cap *= l.slot_count;
        return cap;
    }

    TimerId TimingWheel::InsertNewTask(TimePoint expire, uint32_t interval,
                                       bool repeating, std::function<void()> cb)
    {
    ...
    void TimingWheel::InsertTask(const std::shared_ptr<TimerTask>& task)
    {
        ...
        int64_t distance = std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    void TimingWheel::WorkerLoop()
    {
        ...
        while (!stop_.load(std::memory_order_acquire))
        {
            if (task_map_.empty() && expired_tasks_.empty())
            {
                cv_.wait(lock, [this]
                {
                    return stop_.load(std::memory_order_acquire) ||
                        !task_map_.empty() || !expired_tasks_.empty();
                });
            ...
            if (!expired_tasks_.empty())
            {
                ...
                for (auto& t : to_run)
                {
                    if (!t->is_repeating) task_map_.erase(t->id);
                }
