...
namespace tyke::context
{
    ...
    public:
        ...
        static std::pair<ContextPtr, std::function<void()>> WithDeadline(
            const ContextPtr& parent, std::chrono::system_clock::time_point deadline);

        template <typename Rep, typename Period>
        static std::pair<ContextPtr, std::function<void()>> WithTimeout(
            const ContextPtr& parent, std::chrono::duration<Rep, Period> timeout)
        {
